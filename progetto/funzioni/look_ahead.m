function [v_s,v_t,s_t,t,v_lim,a_s,a_t] = look_ahead(S,v_max,a_max,d_max,v_iniziale,v_finale,s_cong)
% Calcola il profilo di v nel tempo (v_t) e nello spazio (v_t) con
% l'algoritmo look ahead,
% v_lim vettore con i limiti di velocità nell'intervallo

% S(1) posizione iniziale
% S(end) posizione finale
% vmax velocità massima
% amax accelerazione massima
% dmax decelerazione massima
% vi velocità nel punto iniziale
% vf velocità nel punto finale

a_s=[];
a_t=[];
s_t=[S(1)-s_cong];

s_tot=S(end)-S(1)-s_cong;

v = sqrt((v_iniziale^2 * d_max + v_finale^2 * a_max + 2 * a_max * d_max * s_tot) / (a_max + d_max));

% Verifica se il profilo è trapezoidale o triangolare
if v < v_max
    % Profilo triangolare (il robot non raggiunge v_max)
    v_max_eff = sqrt((v_iniziale^2 * d_max + v_finale^2 * a_max + 2 * a_max * d_max * s_tot) / (a_max + d_max));
    % Tempo di accelerazione e decelerazione
    t_a = (v_max_eff-v_iniziale) / a_max;
    t_d = (v_max_eff-v_finale) / d_max ; 
    t_c = 0;
    
    s_a = (v_max_eff+v_iniziale) / 2 * t_a;
    s_d = (v_finale+v_max_eff) / 2 * t_d;
    s_c = 0;
else
    % Profilo trapezoidale
    v_max_eff = v_max;
    % Tempo di accelerazione e decelerazione
    t_a = (v_max_eff-v_iniziale) / a_max;
    t_d = (v_max_eff-v_finale) / d_max ; 

    s_a = (v_max_eff+v_iniziale) / 2 * t_a;
    s_d = (v_finale+v_max_eff) / 2 * t_d;

    t_c = (s_tot-s_a-s_d) / v_max_eff; % Tempo a velocità costante
    s_c = v_max_eff * t_c;
end


% Tempo totale
t_tot = t_a + t_c + t_d;

% Creazione di punti per spazio e tempo
s = S-S(1)-s_cong;
t = linspace(0, t_tot, length(S));
v_s = zeros(size(s));
v_t = zeros(size(t));

% Calcolo della velocità in funzione dello spazio
for i = 1:length(s)
    if s(i) <= s_a
        % Fase di accelerazione
        v_s(i) = sqrt(v_iniziale^2 + 2 * a_max * s(i));
        a_s(i) = a_max;
    elseif s(i) > s_a && s(i) < (s_a + s_c)
        % Velocità costante
        v_s(i) = v_max_eff;
        a_s(i) = 0;
    elseif s(i) >= (s_a + s_c) && s(i) <= (s_a + s_c + s_d) && (v_max_eff^2 + 2 * d_max * ((s_a + s_c) - s(i))) >= 0
        % Fase di decelerazione
        v_s(i) = sqrt(v_max_eff^2 + 2 * d_max * ((s_a+s_c) - s(i)));
        a_s(i) = -d_max;
    else 
%         disp("ERRORE CALCOLO v_s in position: " + i);
%         disp("s(i) = " + s(i))
%         disp("s_a+s_c+s_d = " + (s_a+s_c+s_d))
%         disp("Valore massimo della velocità v_s= " + max(v_s))
        if (v_max_eff^2 + 2 * d_max * ((s_a+s_c) - s(i)))>=0
%             disp("ERRORE RADICE NEGATIVA")
%             i
%             disp("Valore interno alla radice: " + (v_max_eff^2 + 2 * d_max * ((s_a+s_c) - s(i))))
        elseif s(i)>(s_a+s_c) && s(i) < (s_a+s_c+s_d)
%             disp("ERRORE LIMITI POSIZIONE")
%             s(i)
        end
        v_s(i) = v_finale;
    end
end

fc=1;
% Calcolo della velocità in funzione del tempo
for i = 1:length(t)
    if t(i) < t_a
        % Fase di accelerazione
        v_t(i) = v_iniziale + a_max * t(i);
        if i==1
            a_t=[a_t 0];
        else 
            a_t=[a_t a_max];
        end
        s_t(i)=S(1) - s_cong + v_iniziale * t(i) + 0.5 * a_max * t(i)^2;
        
    elseif t(i) <= (t_a + t_c) && t(i) >= t_a
        % Velocità costante
        v_t(i) = v_max_eff;
        a_t=[a_t 0];
        if (fc == 1)
            pos_c = s_t(end);
            fc = 0;
        end
        s_t(i) = s_a + S(1) - s_cong + v_max_eff * (t(i) - t_a);
    else
        % Fase di decelerazione
        v_t(i) = max(v_t) + d_max * ((t_a + t_c) - t(i));
        if (i==length(t)) % controllo se la velocità del tratto successivo è uguale alla velocità dell'ultimo punto di questo tratto
            a_t=[a_t 0];
        else
            a_t=[a_t -d_max]; 
        end
        s_t(i) = s_a + s_c + S(1) - s_cong - v_max_eff * ((t_a + t_c) - t(i)) - 0.5 * d_max * ((t_a + t_c) - t(i))^2;
    end
end

% creo il vettore con le velocità limite da plottare
for i=1:length(S)
    if i == 1
        v_lim(i)=v_iniziale;
    elseif i == length(S)
        v_lim(i)=v_finale;
    else
        v_lim(i)=v_max;
    end
end

% PLOT DI DEBUG
% figure
% subplot(2,2,1)
% plot(t,gradient(s_t,t),'b',t,v_t,'r')
% legend("v numerica","v_t calcolata",'Location','Best')
% grid on
% legend()
% subplot(2,2,2)
% plot(s,v_s,"DisplayName","v_s")
% grid on
% legend()
% subplot(2,2,3)
% plot(t,s_t,"DisplayName","s")
% grid on
% legend()
% subplot(2,2,4)
% plot(t,a_t,"DisplayName","a_s")
% grid on
% legend()


end