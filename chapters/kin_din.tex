...
\[
\begin{cases}
	\alpha=acos(\frac{(x^2+y^2-l_1^2-l_2^2)}{(2l_1l_2)})\\
	\beta=atan(\frac{y}{x})-atan(\frac{l_2sin(q_2)}{l_1+l_2cos(q_2)})\\
	s=h-z\\
	\gamma=g-q_1-q_2\\
\end{cases}\].
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{pos_giunti}
...
\begin{figure}[htbp!]
    \centering
    \includegraphics[width=0.7\textwidth]{vel_giunti}
...
\lstinputlisting[language=Octave,firstline=12,lastline=19]{media/2_kin_din/3_din_inv/dinamica_SCARA_jac_inv.m}

nella quale $ S_e $ cinematica estesa del manipolatore corrisponde alla seguente matrice:
\[
\begin{bmatrix}
...
mentre la derivata dello Jacobiano esteso $ \Dot{J_e} $:
\[
\begin{bmatrix}
...
\lstinputlisting[language=Octave,firstline=42,lastline=76]{media/2_kin_din/3_din_inv/dinamica_SCARA_jac_inv.m}
...
Di seguito si riportano i risultati della dinamica inversa.
